...
static void run_client(void) {
    ...
    for (int i = 0; i < num_client_threads; i++) {
        ...
        printf("Thread %d: tx_cnt=%lld rx_cnt=%lld lost_pkt_cnt=%lld loss_rate=%.2f%%\n",
               i, thread_data[i].tx_cnt, thread_data[i].rx_cnt, thread_data[i].lost_pkt_cnt,
               thread_data[i].tx_cnt > 0 ? 100.0 * thread_data[i].lost_pkt_cnt / thread_data[i].tx_cnt : 0.0);
    }
    ...
    printf("Total: tx_cnt=%lld rx_cnt=%lld lost_pkt_cnt=%lld loss_rate=%.2f%%\n",
           total_tx, total_rx, total_lost, total_tx > 0 ? 100.0 * total_lost / total_tx : 0.0);
