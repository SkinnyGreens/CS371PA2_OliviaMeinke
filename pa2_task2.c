 ...
 *   Student #1: Olivia Nichole Meinke
 *   Student #2:
 *   Student #3:
 *
 * - UDP sockets
...
typedef struct {
    ...
    long long tx_cnt;   // new packets sent
    long long rx_cnt;   // acks received 
...
static void run_client(void) {
    ...
    for (int i = 0; i < num_client_threads; i++) {
        ...
        long long lost = thread_data[i].tx_cnt - thread_data[i].rx_cnt;
        printf("Thread %d: tx_cnt=%lld rx_cnt=%lld lost=%lld loss_rate=%.2f%%\n",
               i, thread_data[i].tx_cnt, thread_data[i].rx_cnt, lost,
               thread_data[i].tx_cnt > 0 ? 100.0 * lost / thread_data[i].tx_cnt : 0.0);
    }

    gettimeofday(&end_time, NULL);
    double elapsed_sec = (end_time.tv_sec - start_time.tv_sec) + (end_time.tv_usec - start_time.tv_usec) / 1000000.0;
    double throughput = elapsed_sec > 0 ? (double)total_rx / elapsed_sec : 0.0;

    printf("Total: tx_cnt=%lld rx_cnt=%lld lost=%lld loss_rate=%.2f%%\n",
           total_tx, total_rx, total_tx - total_rx, total_tx > 0 ? 100.0 * (total_tx - total_rx) / total_tx : 0.0);
    printf("Elapsed: %.3f sec  Throughput: %.1f packets/sec\n", elapsed_sec, throughput);
}
